 ...
 */
static uint8_t rx_ringbuf_array[RX_RINGBUF_SIZE];

/**
 * @brief   Ring buffer data array for TX DMA
 * @note    would printf error if use local variable
 */
__attribute__((section(".dmaBuffer"))) static uint8_t tx_ringbuf_array[TX_RINGBUF_SIZE];

/**
 * @brief   uart dma recieve buffer
 */
__attribute__((section(".dmaBuffer"))) static uint8_t recv_buf[RECV_SIZE];
...
{
    uint32_t sendLen;
    sendLen = lwrb_write(&tx_ringbuf, pData, len); /* Write data to TX buffer for loopback */
...
uint32_t uart_printf(UART_HandleTypeDef *huart, const char *fmt, ...)
{
    ...
    return uart_send_dma(huart, (uint8_t *)buf, len);
}
